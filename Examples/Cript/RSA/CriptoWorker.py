...
class CriptoWorker():
    ...
    def createKey(self,
        ...
        # Приватный ключ
        if privateKeyPwd == True:  # с паролем
            private_key = key.export_key(passphrase=secret,
                                         pkcs=8,
                                         protection="scryptAndAES128-CBC")
        elif privateKeyPwd == False:  # без пароля
    ...
    def createSignature(self, raw_message: bytes, aliseprivate: bytes,
                        bobpublic: bytes) -> bytes:
        ...
        Returns:
            bytes: [Сигнатура сообщения]
        """
        plaintext = raw_message
        # Подписываем цифровой подписью Алисы
        privatekey = RSA.importKey(aliseprivate)
    ...
    def checkSignature(self, message: dict, decript_message: bytes,
                       bobprivate: bytes, alisepublic: bytes) -> bool:
        """Проверка сигнатуры.
        ...
        privatekey = RSA.importKey(bobprivate)
    ...
    def createMsg(self, raw_message: bytes, aliseprivate: bytes,
                  bobpublic: bytes) -> dict:
        ...
        # Создаем сигнатуру
        signature = self.createSignature(raw_message, aliseprivate, bobpublic)
    ...
    def decryptData(self, message: dict, bobprivate: bytes,
                    alisepublic: bytes) -> list:
        """Дешифрование данных.
        ...
        # Провека сигнатуры
        verification = self.checkSignature(message, plaintext, bobprivate,
                                           alisepublic)
...
cript = CriptoWorker()
alise = cript.createKey("alise")
bob = cript.createKey("bob")
cripto_mes = cript.createMsg(message, alise["private"], bob["public"])

cript.saveMsgToFile(cripto_mes, "file.txt")
a = cript.readFileToMsg("file.txt")
print(cripto_mes == a)
d = cript.decryptData(a, bob["private"], alise["public"])
print(f"Соотбщение: {d[0].decode()} \nВерификация: {d[1]}")