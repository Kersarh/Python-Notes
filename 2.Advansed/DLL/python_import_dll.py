...
>>> Для компиляции main.go необходим установленый GoLang <<<
'''
...
# Подключение сфоих файлов
'''
Исходный код написан на языке GO.
Для сборки используйте bat файл.
Go должен быть усановлен.
'''
# Пример 1
...
print(x)
'''
Бывает что необходимо получить доступ к функции или атрибуту DLL, имя которого Python не распознает. На этот случай имеется функция:
