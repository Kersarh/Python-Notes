...
# for x in range(5) - объявляется таже самая переменная x, а после размещается итерируемый объект range(10).
...
# Действие может выполняться с условием if которое указывается после цикла for и проверяется на каждой итерации.
