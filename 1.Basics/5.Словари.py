...
Объявляются словарь в фигурных скобках {},
и доступ к данным осуществляется по ключу.
"""
...
# таким образом ключей для одного значения может быть много
dataname = "user"
...
# Важно
# Попытка обратиться к значению по ключу,
# которого нет в словаре, вызовет исключение KeyError.
...
sumdic = {"name": {"first": "Иван", "second": "Иванов"}, "job": ["Админ", "Программер"]}
print("Пользователь: ", sumdic["name"]["second"], "\tДолжность: ", sumdic["job"][0])
# -----------------------------------------------
...
# В этом примере, мы также используем словарь словарь "dic" и строку "text2".
