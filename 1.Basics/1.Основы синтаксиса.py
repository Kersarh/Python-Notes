...
"""
или в блоке ограниченном тремя апострофами
...
i = 10
txt = "Привет мир!"
# При назначении переменная имеет тип советующий значению.
# Так i является числом (int), а txt строкой (str).
...
# Поскольку существует только одно значение,
# на которое ссылаются переменные i и i2.
...
# аналогичен bytes с тем отличием, что может изменяться
# set - неупорядоченная коллекция уникальных объектов
# frozen set - то же самое, что и set, только не может изменяться (immutable)
"""
Арифметические операции
a + b  # Сложение. Складывает а и b и присваивает значение переменной a
a - b  # Вычитание
a * b  # Умножение
a / b  # Деление
...
# Отступы
"""
Большинство языков программирования, таких как C, C++, Java,
используют фигурные скобки {} для определения блока кода.
В отличии от них, Python использует отступы.
В Python, как правило, используются четыре пробела для выделения блока кода
"""
if True:
    s = "World"
    print("Hello", s)
...
# if True: s = 'World'; print('Hello', s)
# Это не вызовет ошибок однако это является плохой практикой
# потому что снижает читабельность вашего кода.
