...
Генератор можно использовать для итерации в цикле for, но нельзя проиндексировать, так как генератор 
'''
...
print("next(val) =",next(val)) # Первое образение к генератору
